...
function diffMediaRules(ourRule, theirRule) {
	if (theirRule.mediaText != ourRule.mediaText) {
		mediaRuleDiff.mediaText = theirRule.mediaText;
	}
	var rulesListDiff = diffRulesLists(ourRule.rules, theirRule.rules);
	if (rulesListDiff) {
		mediaRuleDirty = true;
...
function diffRulesLists(ourRules, theirRules) {
	var rulesDiff = [],
		skip = 0,
	...
	for (var i = 0; i < minLength; i++) {
		...
		if (ourRule.type != theirRule.type) {
			...
			var makeDiff = ourRule.type == 'rule' ? diffRules :
				ourRule.type == 'keyframes' ? diffKeyframesRules :
				ourRule.type == 'media' ? diffMediaRules :
				Error;
...
Sheet.prototype.getDiff = function(sheet) {
	var ourRules = this.cssRules,
		theirRules = sheet.cssRules || [];
	// todo: mediatext
	return {
		rules: diffRulesLists(ourRules, theirRules)
	};
};
...
function cssRuleToObject(rule) {
	if (rule.type == 'rule') {
		...
		return {
			type: 'media',
			mediaText: rule.media,
			rules: rule.rules.map(cssRuleToObject)
		};
